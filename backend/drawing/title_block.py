...
def draw_title_block(fig, sheet_num, sheet_title, calc, project_info=None):
    ...
    fig.patches.extend([
        fig.add_artist(fig.patch) if False else None  # placeholder
    ]) if False else None

    # Bottom bar with sheet info
    fig.text(0.5, 0.02,
             f'SHEET {sheet_num}  |  {sheet_title}  |  {format_feet_inches(W)} × {format_feet_inches(D)}  |  simpleblueprints.xyz',
