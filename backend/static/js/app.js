...
// Zone management functions
  ...
  const addZone = (parentId, edge) => {
    ...
    // S105: no stored auto-label. sectionName() derives "Deck B/C/D" from
    // position, so a delete renumbers cleanly and old rows upgrade for free.
    return {
      ...prev,
  ...
  const addCutout = (parentId, edge) => {
    ...
    if (!defaults) return prev;
    defaults.id = prev.nextZoneId;
    // S105: derived, not stored. See the note in addZone above.
    return {
      ...prev,
      zones: prev.zones.concat([defaults]),
      activeZone: prev.nextZoneId,
...
// S64: Stair management functions
  ...
  // S64: Batch update multiple stair fields in one state change (for drag)
  const updateStairFields = (stairId, fields) => setP(prev => {
    var next = { ...prev, deckStairs: (prev.deckStairs || []).map(function(s) {
      if (s.id !== stairId) return s;
...
// Provide deckWidth/deckDepth aliases for zoneUtils
  ...
  // S103: this used to patch railLen here, in app.js only, using a blunt
  // subtraction of each stair's full width. calcStructure now derives railLen
  // from the same exposed edges deck3d.js draws (engine.js
  // railLengthFromDrawing), so the override is gone and the rule lives in one
  // place. Kept as an alias so the consumers below do not churn.
  const cAdj = c;
  ...
  const [shapeHFlip, setShapeHFlip] = useState(false);
  const [shapeVFlip, setShapeVFlip] = useState(false);
  // S48: Reset preview when exiting compare mode
  useEffect(() => { if (!compareMode) { setPreviewIdx(null); window._previewShapeIndex = null; setAutoConfirmDismissed(false); setShapeHFlip(false); setShapeVFlip(false); } }, [compareMode]);
  ...
  // S53: Compute effective orientation for shape display
  var _streetSide = (window._rankingResult && window._rankingResult.streetSide) || (window._shapeCompareData && window._shapeCompareData.extractResult ? window._shapeCompareData.extractResult.streetSide : null);
  // Auto-mirror for streetSide="top" (180 rotation + SVG Y-flip = net X-flip)
  var _autoMirror = _streetSide === "top";
  // XOR: shapeHFlip toggles the auto-mirror on/off
  var _effectiveHFlip = shapeHFlip !== _autoMirror;
            ...
            <div style={{ background: step === 0 ? br.cr : (view === "3d" ? "transparent" : br.cr), border: step === 0 || view !== "3d" ? `1px solid ${br.bd}` : "none", borderRadius: 6, padding: step === 0 ? 8 : (view === "3d" ? 0 : 12), minHeight: 320 }}>
              ...
              {step === 0 && compareMode && sitePlanB64 && SurveyPreview && <div>
                ...
                {window._rankingResult && window._rankingResult.confidence === "high" && previewIdx != null && !autoConfirmDismissed && window._shapeCompareData ? (() => {
                  ...
                  var rv = window._rotateVertsForDisplay ? window._rotateVertsForDisplay(bestCand.vertices, streetSide, _effectiveHFlip, _effectiveVFlip) : bestCand.vertices;
                  ...
                  return <div>
                    ...
                    <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                      ...
                      <div style={{ flex: "1 1 45%", display: "flex", flexDirection: "column", alignItems: "center" }}>
                        ...
                        <button onClick={() => { if (window._trackEvent) window._trackEvent('auto_confirm_action', { action: 'accept', shape_index: bestIdx }); if (window._selectShape) window._selectShape(bestIdx); }} style={{
                        ...
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
                          ...
                          <button onClick={() => { if (window._trackEvent) window._trackEvent('user_flip', { direction: 'horizontal' }); setShapeHFlip(!shapeHFlip); }} style={{ fontSize: 9, fontFamily: mono, color: (_effectiveHFlip) ? "#2e7d32" : br.mu, background: (_effectiveHFlip) ? "#f0fdf4" : "none", border: "1px solid " + ((_effectiveHFlip) ? "#2e7d32" : br.bd), borderRadius: 4, padding: "2px 8px", cursor: "pointer" }}>{"\u2194"} Flip</button>
                          <button onClick={() => { if (window._trackEvent) window._trackEvent('user_flip', { direction: 'vertical' }); setShapeVFlip(!shapeVFlip); }} style={{ fontSize: 9, fontFamily: mono, color: _effectiveVFlip ? "#2e7d32" : br.mu, background: _effectiveVFlip ? "#f0fdf4" : "none", border: "1px solid " + (_effectiveVFlip ? "#2e7d32" : br.bd), borderRadius: 4, padding: "2px 8px", cursor: "pointer" }}>{"\u2195"} Flip</button>
                        </div>
                        <button onClick={() => { if (window._trackEvent) window._trackEvent('auto_confirm_action', { action: 'dismiss' }); setAutoConfirmDismissed(true); }} style={{
                ...
                {previewIdx == null ? <div>
                  ...
                  <div style={{ fontSize: 8, fontWeight: 700, color: br.mu, fontFamily: mono, letterSpacing: "1px", textTransform: "uppercase", marginBottom: 6 }}>Proposed Shapes</div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "8px 14px", background: "#fefce8", borderRadius: 6, border: "1px solid #fde68a", marginBottom: 10, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 10, fontFamily: mono, color: "#92400e" }}>Shapes don't match?</span>
                    <button onClick={() => { if (window._trackEvent) window._trackEvent('user_flip', { direction: 'horizontal' }); setShapeHFlip(!shapeHFlip); }} style={{ fontSize: 9, fontFamily: mono, color: _effectiveHFlip ? "#2e7d32" : "#92400e", background: _effectiveHFlip ? "#f0fdf4" : "none", border: "1px solid " + (_effectiveHFlip ? "#2e7d32" : "#fde68a"), borderRadius: 4, padding: "3px 10px", cursor: "pointer", fontWeight: 600 }}>{"\u2194"} Flip</button>
                    <button onClick={() => { if (window._trackEvent) window._trackEvent('user_flip', { direction: 'vertical' }); setShapeVFlip(!shapeVFlip); }} style={{ fontSize: 9, fontFamily: mono, color: _effectiveVFlip ? "#2e7d32" : "#92400e", background: _effectiveVFlip ? "#f0fdf4" : "none", border: "1px solid " + (_effectiveVFlip ? "#2e7d32" : "#fde68a"), borderRadius: 4, padding: "3px 10px", cursor: "pointer", fontWeight: 600 }}>{"\u2195"} Flip</button>
                    <span style={{ color: "#fde68a", fontSize: 10 }}>|</span>
                  ...
                  <CompareShapes candidates={window._shapeCompareData ? window._shapeCompareData.candidates : []} previewIdx={previewIdx} streetSide={_streetSide} hFlip={_effectiveHFlip} vFlip={_effectiveVFlip} />
                  ...
                  <div style={{ flex: "1 1 45%", display: "flex", flexDirection: "column" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "6px 10px", background: "#fefce8", borderRadius: 6, border: "1px solid #fde68a", marginBottom: 8, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 9, fontFamily: mono, color: "#92400e" }}>Shapes don't match?</span>
                      <button onClick={() => { if (window._trackEvent) window._trackEvent('user_flip', { direction: 'horizontal' }); setShapeHFlip(!shapeHFlip); }} style={{ fontSize: 8, fontFamily: mono, color: _effectiveHFlip ? "#2e7d32" : "#92400e", background: _effectiveHFlip ? "#f0fdf4" : "none", border: "1px solid " + (_effectiveHFlip ? "#2e7d32" : "#fde68a"), borderRadius: 4, padding: "2px 8px", cursor: "pointer", fontWeight: 600 }}>{"\u2194"} Flip</button>
                      <button onClick={() => { if (window._trackEvent) window._trackEvent('user_flip', { direction: 'vertical' }); setShapeVFlip(!shapeVFlip); }} style={{ fontSize: 8, fontFamily: mono, color: _effectiveVFlip ? "#2e7d32" : "#92400e", background: _effectiveVFlip ? "#f0fdf4" : "none", border: "1px solid " + (_effectiveVFlip ? "#2e7d32" : "#fde68a"), borderRadius: 4, padding: "2px 8px", cursor: "pointer", fontWeight: 600 }}>{"\u2195"} Flip</button>
                      <span style={{ color: "#fde68a", fontSize: 9 }}>|</span>
...
// {zoneMode === "select" && <>Drag the <span style={{ color: "#3d5a2e", fontWeight: 700 }}>green</span> handle to slide the deck   Click <span style={{ color: "#c62828", fontWeight: 700 }}>stairs</span> to select, drag to move, grab <span style={{ color: "#3d5a2e", fontWeight: 700 }}>{"\u21BB"}</span> to rotate</>}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 6, marginTop: 12 }}>
              {[["Area", `${zc ? zc.totalArea : c.area} SF`, br.dk], ["Joists", c.joistSize, br.bl], ["Beam", c.beamSize.replace("3-ply ", "3\u00D7").replace("2-ply ", "2\u00D7"), br.ac], ["Posts", `${c.postSize}\u00D7${zc ? c.totalPosts + zc.extraPosts : c.totalPosts}`, "#8B6508"], ["Footings", `${c.fDiam}"\u00D8\u00D7${zc ? c.nF + zc.extraFootings : c.nF}`, "#777"], ["Est. Cost", `$${(zc ? m.total + zc.extraTotal : m.total).toFixed(0)}`, br.gn]].map(([l, v, cl]) => (
