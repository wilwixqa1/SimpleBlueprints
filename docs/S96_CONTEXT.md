# SimpleBlueprints — MASTER CONTEXT & TODO (start-here; written at S96 close)

Single "hit the ground running" doc. Read the **S96 RETROSPECTIVE (just below)**
and §0/§1 first, then work §4. Self-contained.
Supersedes docs/S95_CONTEXT.md. NUMBERING: this session was "S96" (Will's
counter); a PARALLEL session is using "S95" and is porting the mock homepage to
production (landed `5a6acb8`) — see the retrospective. Sections §0–§8 below are
the S94-era baseline, still accurate EXCEPT where the retrospective updates them.
...
## S96 RETROSPECTIVE & HANDOFF (READ FIRST — updates §0–§8 below)

**Shipped in S96 (all 14-check gate green locally; pushed on top of `5a6acb8`;
CI confirmed):**
1. **Details sheet added to the structural golden** (was a known gap: the S94
   details rework had NO regression guard). +5 fingerprints (15→20); existing
   site/plan/framing fingerprints byte-identical. Details fingerprint ~48KB/config
   (≈2× framing, acceptable — nowhere near the elevations blowup that got them
   excluded). [tests/pdf/golden_structural.py + .json] → resolves §4/P2 "details
   in the golden."
2. **Steel notched-deck beam FIX** (was §4/P4). `calculate_steel_structure`
   returned early with a straight, even-spaced beam — it skipped the cutout-aware
   layout, so a notch could strand a steel post and the beam ignored the notch.
   Steel now shares `compute_beam_layout` (pure geometry; steel keeps its own
   2x11 single/double SIZING). Verified: FLAT steel post positions BYTE-IDENTICAL
   to before (`_legacy_posts` == the old formula) → plain steel decks unchanged;
   notched steel now `stepped`, no post stranded. The framing drawing renders the
   stepped steel beam with NO drawing change (it branches on `stepped`, not
   material). Added steel-notch regression cases to test_notch_posts.py.
   `cantilever_max` uses the code's R507.5 span/4 convention (calc_engine ~L148);
   refine if CCRR-0313 specifies a steel-specific allowance.
   [backend/drawing/calc_engine.py + tests/test_notch_posts.py]

**Decisions captured (see §6):** North star RESOLVED = BOTH audiences, GC-leaning.
Permit-framing-vs-competitor-style question PARKED with full context.

**VIEW TOOL — important update to §0/§5:** was DOWN essentially all of S96, for
BOTH self-generated renders AND Will's UPLOADS (it worked on the very first
uploads of the session, then returned blank placeholders for everything after;
flatten-to-RGB and JPEG re-encode did NOT recover it; OCR can't read the small
drawing text). Clarification for the doc: normally the tool is reliable on Will's
UPLOADS and only flaky on self-generated renders — but S96 proves it can fail on
uploads too. When it's down: verify NUMERICALLY (coords, pixel diffs, primitive
extraction), reason from CODE + the exact config, and/or have Will screenshot
(his side usually renders). NEVER block on it; NEVER let a claim rest on a render
you couldn't see.

**AUDIT (FE↔BE connectivity) — Will's ask, IN PROGRESS, method lesson learned.**
Goal: every FE control must MEANINGFULLY change the PDF, and every BE knob the PDF
generator honors must be FE-reachable. KEY LESSON: a name-match ("does the BE read
this key?") is the WRONG instrument — it is blind to the "control is read but the
output doesn't change" case, which is exactly what Will cares about (the railType
bug is the archetype). The RIGHT method is DIFFERENTIAL: flip each control to two
values, regenerate the PDF, and pixel/structural-diff; zero diff ⇒ dead control.
This works even with the view tool down. First differential runs found: stair
TEMPLATE is NOT dead (straight/lLeft/switchback/wrapAround all change the plan
~4% + elevations change too); framingType, slopeDirection, beamType all DO have
FE controls (the first name-match FALSELY flagged them because the wizard sets
params via a `u("key", val)` updater, NOT `field=` — so extract FE keys from
`u(...)` + the canvas/form flows, not just `field=`). Remaining leads to verify:
landingDepth / runSplit / stairGap (read in exactly 1 backend place each — confirm
that read is MEANINGFUL, not vestigial), and lotArea (declared in DeckParams but
seemingly superseded by a computed area). NEXT: build the full differential map
with the corrected FE extraction; put Will's eyes only on the diffs where
"changed" ≠ "correct."

**STAIR TEMPLATES — TOP OF NEXT SESSION (needs the view tool).** Will wants ALL
templates (straight, lLeft, lRight, switchback, wrapAround, wideLanding) verified
to render correctly in the blueprints, not just wraparound. Concrete S96 findings:
- **REAL BUG (z-order):** the deck decking fill is painted OVER the stair 'DOWN'
  arrows/labels, so descent direction is hidden (the arrows are likely correct
  underneath). Same matplotlib patch-vs-line zorder issue as the S94 notch fix
  (§5). Fixable, and pixel-diff-verifiable without the view tool.
- **Legibility:** you can't tell which stair runs connect to the DECK (top) vs the
  GROUND (bottom); multi-run 'DOWN' labels (one per run, 72% along the run) point
  different ways on a wrap/switchback. Consider distinguishing deck-level vs
  grade-level elements.
- **Concrete landing:** for a RAISED deck with a folding (wrap/switchback) stair,
  the grade concrete pad CORRECTLY sits at grade UNDER the deck's overhead
  footprint → it appears "on the deck" in the top-down plan. Will is fine with
  under-deck if that's how it'd really be built (it is). Represent deck-level vs
  grade-level distinctly so it reads right.
- **CAUTION (bad fixture):** S96's wrap test used an UNREALISTIC config (wrapAround
  on a plain front edge, offset 0, NO cutout/opening zone) → confusing output.
  Per §0, a weird render is usually a bad fixture. Test templates with REALISTIC
  configs (proper opening, room to fold).

**PAT:** Will provided a GitHub PAT in the S96 chat to push. **ROTATE IT** now that
the session is done (reminded in-session). Used transiently, never stored, swept.

**`main` keeps moving:** the parallel S95 (homepage) session pushed mid-S96
(`5a6acb8`). Always `git fetch` + check the new commits' scope before pushing;
S96's changes (calc_engine/tests/golden/docs) had ZERO overlap with the homepage
port, so the rebase was clean and no cache-buster conflict (S96 touched no
...
**Still pending:**
- **NORTH STAR: homeowner vs contractor — RESOLVED (Will, S96): BOTH.**
  Both are target audiences. General contractors are weighted slightly HIGHER
  (they give repeat business), BUT the product must still meet the needs of
  the average DIY homeowner. Practical reading for design calls: favor
  contractor/buildability value (cut lists, member tags, shop-quality
  drawings) as a differentiator, WITHOUT making the core flow or the permit
  sheets too complex for a DIY homeowner to use and submit. This retires the
  old MASTER_CONTEXT/S84 (contractors) vs SBP_GTM_PLAN (homeowners) conflict.
  It also informs the parked framing-sheet question below (a both-audiences
  product argues for: keep the permit sheet clean for the homeowner + add a
  separate build/cut-list packet for the contractor).
- **PERMIT FRAMING SHEET: leave as-is vs. restyle toward the competitor
  images? (UNRESOLVED — Will deferred in S96, "come back to later.")** Full
  discussion so next session doesn't re-derive it:
  - *What triggered it:* Will shared the two competitor screenshots (the P0
    references, now seen concretely) and asked whether OUR framing output is
    "good enough" or should look "closer to the reference images." He had NOT
    realized our existing framing sheet WAS our deck framing — because on
    SIMPLE decks it has no standalone page (see next point), so it reads as
    "we don't do this."
  - *Verified fact (S96, from generated output — not just the doc):* we DO
    produce a framing plan. COMPLEX decks (zones>0) get a dedicated sheet
    titled **"A-2 DECK FRAMING"** (e.g. zones_stairs_lcr permit p3). SIMPLE
    decks (no zones) have **no standalone framing page** — it's folded into
    **"A-1 DECK PLAN & FRAMING"** (basic_rect_ledger permit p2). THIS is why
    it's easy to think we don't produce framing: test a plain rectangle and
    you never see a page called "framing." (Confirms the doc's "existing
    framing sheet satisfies the permit requirement" claim — it's correct AND
    verified; just non-obvious on simple decks.)
  - *What ours carries:* joist size+spacing (2x8 @ 16" O.C.), beam, 6x6 posts
    on piers, ledger + fastener callout, mid-span blocking, load table,
    Simpson ties/hangers — i.e. the PPRBD framing checklist (material, sizes,
    spacing, connections). Same GROUP-callout style as the two APPROVED
    reference sets (Ilaria/Loucks). Read from the drawing text layer; the
    render would not display in-session (sandbox-render flakiness), so the
    VISUAL verdict rests on Will — he looked at framing_complex.png /
    framing_simple.png and accepted them as our framing once he knew what
    they were.
  - *What the competitor images add (= P0):* image 1 = a STANDALONE labeled
    cut-list/material-list sheet (per-piece tags J1.., cut list with per-piece
    LENGTHS, material totals, summary block: deck area / perimeter / concrete
    volume / concrete bags). image 2 = an interactive 3D framing view with a
    label chip on every member (J1–J15 joists, S1 stringers) + a "Switch To
    PRO" button (a competitor app UI). Both are BUILD info, not PERMIT info;
    NEITHER approved set has any of it.
  - *Claude's recommendation (S96, Will did not yet accept or reject):* keep
    the permit framing sheet EXACTLY as-is (don't clutter the review-critical
    page; it's already at the approved bar), and add the competitor value as
    a SEPARATE cut-list / shop-drawing sheet — which is literally what image 1
    already is (a standalone sheet, not the permit page with tags bolted on).
    The labeled 3D view (image 2) is a bigger, separate FRONTEND job → later
    phase. This hits both north stars: permit already met + buildability added
    as a differentiator (like our guard-rail detail).
  - *The still-open part:* only whether to ALSO restyle the PERMIT framing
    sheet toward the competitor look. The separate-cut-list-sheet path (the
    bulk of P0) can proceed WITHOUT resolving this. Revisit if/when we settle
    the north star (a contractor-first product might justify a busier,
    build-oriented permit sheet; a homeowner-first product argues for keeping
    the permit sheet clean + a separate build packet).
